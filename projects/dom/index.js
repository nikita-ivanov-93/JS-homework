    ...
    for (const node of elem.childNodes) {
      if (node.nodeType === 3) {
        ...
        if (statResult.tags.hasOwnProperty(node.nodeName)) {
          statResult.tags[node.nodeName]++;
        } else {
          statResult.tags[node.nodeName] = 1;
        }
        for (const classItem of node.classList) {
          // eslint-disable-next-line no-prototype-builtins
          if (statResult.classes.hasOwnProperty(`${classItem}`)) {
            statResult.classes[`${classItem}`]++;
          } else {
            statResult.classes[`${classItem}`] = 1;
          }
        }
        recursive(node);
      }
